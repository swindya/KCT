...
'Response.Write(userid & "---" & namauserx)
	...
	If NOT Request.Form("pwd1") is nothing Then
		pwdx = Request.Form("pwd1")
	End If
...
<%
			...
			If pwdx.length() < 3 Then
				SQL0 = "UPDATE [user] SET nama='" & namax & "',levelakses='" & levelx & "' WHERE (username='" & usernamax & "')"			
			ElseIf pwdx.length() > 2 Then
				SQL0 = "UPDATE [user] SET nama='" & namax & "',paswd=HASHBYTES('SHA2_256','" & _
						pwdx & "'),levelakses='" & levelx & "' WHERE (username='" & usernamax & "')"
			End If
