...
<%
	...
	If NOT Request.Form("tgldari") is nothing Then
		tgldarix = Request.Form("tgldari")
	End If
	Dim tglarr(3) as String 
	tglarr = tgldarix.Split("-")
	tgldarixx = tglarr(2) & "-" & tglarr(1) & "-" & tglarr(0)
	
	If NOT Request.Form("tglsd") is nothing Then
		tglsdx = Request.Form("tglsd")
	End If 
	tglarr = tglsdx.Split("-")
	tglsdxx = tglarr(2) & "-" & tglarr(1) & "-" & tglarr(0)
...
<%	
	...
	'SQL1 = "SELECT * FROM kct WHERE (tgl >= '" & DateTime.Parse(tgldarix) & "' AND tgl <= '" & DateTime.Parse(tglsdx) & "') ORDER BY tgl DESC"
	SQL1 = "SELECT * FROM kct WHERE (tgl >= '" & tgldarixx & "' AND tgl <= '" & tglsdxx & "') ORDER BY tgl DESC"
	'SQL1 = "SELECT * FROM kct WHERE (tgl >= '2000-01-01' AND tgl <= '2021-03-20') ORDER BY tglefektif DESC"
'response.write(SQL1)
	'SQL1 = "SELECT * FROM kct WHERE (tglefektif >= '" & tgldarix & "') ORDER BY tglefektif DESC"
	...
	If myReader.HasRows Then
		While myReader.Read()
			...
			If jenisoutletx(a)="CAB" Then
				kcx(a) = brklnx(a)
			Else If jenisoutletx(a)="KCP" Then
				kcpx(a) = brklnx(a)
			End if
	...
	for a=1 to jmldata 
		If (jenisoutletx(a) = "KCP") Then
			SQL2 = "SELECT kln.kln_branch AS namakcp, branch.br_nama AS namacabang, kln.kln_nama FROM branch INNER JOIN " & _
                    "kln ON branch.br_id = kln.kln_branch WHERE (kln.kln_nama='" & brklnx(a) & "')"
			myConn.Open()
			myCmd.CommandText = SQL2
			myReader = myCmd.ExecuteReader()
			If myReader.HasRows Then
				While myReader.Read()
					kcx(a) = myReader("namacabang").ToString()
				End While
			End If
			myConn.Close()
		End If
...
<table id="kctbarutab" style="text-align: left; margin-left: 50px; margin-right: auto; margin-top: 0;" width="910px" border="0" cellspacing="0" cellpadding="0">
	...
	<tr height="35px">
		<td style="padding-left: 0px; text-align: left; font-size: 1.0em; vertical-align: top" width="100px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: top" width="780px">
			<FONT face="Calibri" color="#000">
<%
			iDate = Convert.ToDateTime(tgldarix)
			Response.Write(iDate.toString("d MMM yyyy"))
			'Response.Write(tgldarix) 
%>
			</FONT>
			<FONT size="2">
			&nbsp;s/d&nbsp;
			...
			<FONT face="Calibri" color="#000">
<%
			iDate = Convert.ToDateTime(tglsdx)
			Response.Write(iDate.toString("d MMM yyyy"))
			'Response.Write(tglsdx) 
%>
			</FONT>
		</td>
	</tr>
...
<%
				...
				'oDate = 
				'Dim isValidDate As Boolean = IsDate(Convert.ToDateTime(tglendx(a)))
				'Response.Write(isValidDate)
				'if (isValidDate) Then
				'	Response.Write(oDate.toString("d-MM-yyyy"))
				'Else
				'	Response.Write("-")
				'End If
				if (tglendx(a).length() > 5) Then
					oDate = Convert.ToDateTime(tglendx(a))
					If Val(oDate.toString("yyyy")) > 2000 Then
						Response.Write(oDate.toString("d-M-yyyy"))
					End If
				else 
					Response.Write("-")
